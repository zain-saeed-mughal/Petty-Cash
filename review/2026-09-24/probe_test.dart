...
class Manifest implements AssetManifest {
  @override
  List<String> listAssets() => ['review/Inter-Regular.ttf'];
  @override
  dynamic noSuchMethod(Invocation i) => super.noSuchMethod(i);
}
...
void main() {
  ...
  testWidgets('Current screen layout evidence with real fonts', (t) async {
    await t.runAsync(() async {
      gf.assetManifest = Manifest();
      GoogleFonts.config.allowRuntimeFetching = false;
      binding.defaultBinaryMessenger.setMockMessageHandler('flutter/assets', (
        data,
      ) async {
        final key = utf8.decode(data!.buffer.asUint8List());
        final path = key.startsWith('review/')
            ? '${dir.path}/fonts/${key.substring(7)}'
            : '${Directory.current.path}/build/unit_test_assets/$key';
        final file = File(path);
        return await file.exists()
            ? ByteData.sublistView(await file.readAsBytes())
            : null;
      });
      for (final family in ['ReviewRoboto', 'Roboto']) {
        final f = FontLoader(family)
          ..addFont(
            File('${dir.path}/fonts/Roboto-Regular.ttf')
                .readAsBytes()
    ...
    ButtonStyle? normalize(ButtonStyle? s) => s?.copyWith(
      textStyle: WidgetStatePropertyAll(
        (s.textStyle?.resolve({}) ?? const TextStyle()).copyWith(
          fontFamily: 'ReviewRoboto',
        ),
      ),
    );
    final theme = base.copyWith(
      ...
      textButtonTheme: TextButtonThemeData(
        style: normalize(base.textButtonTheme.style),
      ),
      elevatedButtonTheme: ElevatedButtonThemeData(
        style: normalize(base.elevatedButtonTheme.style),
      ),
      outlinedButtonTheme: OutlinedButtonThemeData(
        style: normalize(base.outlinedButtonTheme.style),
      ),
    );
    ...
    final screens = <String, Widget Function()>{
      ...
      'detail': () =>
          RequestDetailScreen(request: rows.first.copyWith(billImageUrl: '')),
    ...
    for (final size in [
      ...
      for (final scale in [
        ...
        for (final entry in screens.entries) {
          a.user = entry.key == 'new_request' || entry.key == 'my_requests'
              ? staff
              : manager;
          final errors = <String>[];
          ...
          Widget home = standalone
              ? (['login', 'detail'].contains(entry.key)
                    ? child
                    : Scaffold(
                        body: Align(
