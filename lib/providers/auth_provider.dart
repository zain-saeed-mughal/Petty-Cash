...
class AuthProvider extends ChangeNotifier {
  ...
  void _watch() {
    ...
    _authEvents = SupabaseService().client?.auth.onAuthStateChange.listen((
      event,
    ) {
      if (event.session == null && _currentUser != null) {
        _currentUser = null;
        unawaited(PushNotificationService().bindUser(null));
        _emit();
      }
    });
  ...
  Future<void> refreshProfile() async {
    ...
    try {
      ...
      if (!_disposed && generation == _generation && _currentUser?.uid == uid) {
  ...
  Future<bool> signUp({
    ...
    _errorMessage = 'Ask an administrator to create your account.';
    _emit();
    return false;
  }
