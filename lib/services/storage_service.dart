...
class StorageService {
 ...
 SupabaseClient get _client=>SupabaseService().client??(throw StateError('Receipt storage is unavailable. Try again.'));
 Future<XFile?> pickReceiptImage({ImageSource source=ImageSource.gallery}) =>
   _picker.pickImage(source:source,maxWidth:1600,maxHeight:1600,imageQuality:85);
 static String imageMime(Uint8List b){
   if(b.length>=3&&b[0]==0xff&&b[1]==0xd8&&b[2]==0xff)return 'image/jpeg';
   if(b.length>=8&&b[0]==0x89&&b[1]==0x50&&b[2]==0x4e&&b[3]==0x47)return 'image/png';
   if(b.length>=12&&String.fromCharCodes(b.take(4))=='RIFF'&&String.fromCharCodes(b.skip(8).take(4))=='WEBP')return 'image/webp';
   throw const FormatException('Choose a JPEG, PNG or WebP receipt image.');
 }
 Future<String> uploadReceiptImage({required Uint8List imageBytes,required String fileName,String? mimeType}) async {
   if(imageBytes.isEmpty||imageBytes.length>maxUploadBytes)throw StateError('Choose a receipt image under 10 MB.');
   ...
   await client.storage.from('receipts').uploadBinary(path,imageBytes,fileOptions:FileOptions(contentType:mime));
 ...
 String? objectPath(String reference){
   ...
   for(final prefix in ['/storage/v1/object/sign/receipts/','/storage/v1/object/public/receipts/','/storage/v1/object/authenticated/receipts/']){
     if(uri.path.startsWith(prefix))return Uri.decodeComponent(uri.path.substring(prefix.length));
   }
   return null;
 }
